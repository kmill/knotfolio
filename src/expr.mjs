...
function is_zero(e) {
  return e instanceof Const && e[0] === 0;
}

function is_one(e) {
  return e instanceof Const && e[0] === 1 && e[1] === 1;
}
...
class Const extends ExprType {
  ...
  neg() {
    return Const.make(-this[0], this[1]);
  }
}
...
class Times extends ExprType {
  simplify() {
    ...
    while (i < this.length) {
      ...
      if (is_zero(this[i])) {
        return this[i];
      } else if (this[i] instanceof Const) {
        c *= this[i][0];
        denom *= this[i][1];
        this.splice(i, 1);
    ...
    if (c !== denom) {
      this.unshift(make_const(c, denom));
    }
    if (this.length === 0) {
      return make_const(1);
    }
...
export function make_const(a, b=1) {
  return Const.make(a, b);
}
...
function toMathPrec(o) {
  if (o instanceof Const) {
    ...
    let neg = false;
    if (o[0] instanceof Const && o[0][0] === -1 && o[0][1] === 1) {
      neg = true;
      o = Array.from(o);
      o.shift();
    }
    ...
    let parts = o.map((part, i) => {
      let neg = false;
      if (i > 0 && part instanceof Times && part[0] instanceof Const && part[0][0] < 0) {
        neg = true;
        part = times(part[0].neg(), part.slice(1));
      } else if (i > 0 && part instanceof Const && part[0] < 0) {
        neg = true;
        part = part.neg();
      }
      return {neg: neg,
